...
class InpaintRequest(BaseModel):
    ...
    refine_steps: int = Field(default=24, ge=1, le=150)
...
class InpaintProgress(UpscaleProgress):
    """Inpaint job progress = the shared upscale shape plus batch fields (a superset,
...
def start_inpaint(req: InpaintRequest) -> InpaintStarted:
    ...
    with _lock:
        if any(j.status == "running" for j in _jobs.values()):
            raise HTTPException(409, messages.INPAINT_ALREADY_RUNNING)
...
@router.get("/{job_id}", response_model=InpaintProgress)
def inpaint_progress(job_id: str) -> InpaintProgress:
    with _lock:
        job = _jobs.get(job_id)
        if job is None:
            raise HTTPException(404, messages.JOB_NOT_FOUND.format(job_id=job_id))
        return InpaintProgress(
            job_id=job.job_id,
            status=job.status,
            phase=job.phase,
